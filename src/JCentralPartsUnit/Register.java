...
public class Register {
    ...
    public void setValue(char[] new_value) { //sets a value to the register
        if (inputEnable)
            if (new_value.length == this.value.length) System.arraycopy(new_value, 0, this.value, 0, this.value.length);
            else System.err.printf("Cannot set value to %s, byte lengths do not match!\r\n", this.name);
        else System.err.printf("Cannot set value to %s, input is disabled!\r\n", this.name);

    }

    public void setBit(int bit, char value) { //sets a specific bit high or low
        if (inputEnable)
            if (value > 1) System.err.println("Cannot set bits higher than 1!");
            else if (bit - 1 > this.value.length || bit <= 0) System.err.println("Cannot set that bit. Your index is too high!");
            else this.value[bit - 1] = value;
    }
    ...
    public char getBit(int bit) {
        if (outputEnable) if (bit - 1 > this.value.length || bit <= 0) {
            System.err.printf("Could not get bit, as bit %d does not exist! Returned 0.\r\n", bit);
            return 0;
        } else return this.value[bit - 1];
        System.err.println("Returned 0, output is disabled");
        return 0;
    }
}
